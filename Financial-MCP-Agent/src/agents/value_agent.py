    ...
    try:
        ...
        llm = ChatOpenAI(
            ...
            max_tokens=6000   # 增加token数量用于详细分析
                ...
                # 2. 获取MCP工具集
        logger.info(
            f"{WAIT_ICON} ValueAgent: Fetching MCP tools..."
        )
        ...
        try:
            ...
            async with get_mcp_tools() as mcp_tools:
                ...
                # 3. 创建ReAct Agent
                logger.info(
                    f"{WAIT_ICON} ValueAgent: Creating ReAct agent..."
                )
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的工具或参数组合，基于可用信息提供尽可能全面的分析。请保持回答简洁，避免冗长的描述性文字"""
                ...
                # 5. 调用ReAct Agent
                logger.info(
                    f"{WAIT_ICON} ValueAgent: Calling ReAct agent..."
                )
                ...
                # 必须在 async with 内执行。
                # 这样工具调用期间 MCP session 不会提前关闭。
                response = await agent.ainvoke(
                    input_data
                )

            # Agent 完整执行完毕后才退出 MCP session
            end_time = time.time()
            ...
            if (
                ...
                logger.info(
                    f"Response messages count: {len(messages)}"
                )
                ...
                logger.info(
                    f"AI messages count: {len(ai_messages)}"
                )
            ...
            logger.info(
                f"{SUCCESS_ICON} ValueAgent: "
                f"Successfully completed valuation analysis."
            )
            ...
            current_metadata[
                "value_agent_executed"
            ] = True

            current_metadata[
                "value_agent_timestamp"
            ] = str(time.time())

            current_metadata[
                "value_agent_execution_time"
