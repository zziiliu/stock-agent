    ...
    try:
        ...
        llm = ChatOpenAI(
            ...
            max_tokens=6000   # 增加token数量用于详细分析
                ...
                # 2. 获取MCP工具集
        logger.info(
            f"{WAIT_ICON} TechnicalAgent: Fetching MCP tools..."
        )
        ...
        try:
            ...
            async with get_mcp_tools() as all_mcp_tools:
                ...
                if not mcp_tools:
                    logger.error(
                        f"{ERROR_ICON} "
                        f"TechnicalAgent: No MCP tools available."
                    )
                    current_data[
                ...
                logger.info(
                    f"{SUCCESS_ICON} "
                    f"TechnicalAgent: Successfully loaded "
                ...
                logger.info(
                    f"{WAIT_ICON} "
                    f"TechnicalAgent: Creating ReAct agent..."
                )
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。"""
                ...
                logger.info(
                    f"{WAIT_ICON} "
                    f"TechnicalAgent: Calling ReAct agent..."
                )
                ...
                # 必须在 async with 作用域内执行 Agent，
                # 确保 MCP session 在工具调用期间保持打开
                response = await agent.ainvoke(
                    input_data
                )

            # Agent 完整执行结束后，才退出 MCP session
            end_time = time.time()
            ...
            logger.info(
                f"{SUCCESS_ICON} "
                f"TechnicalAgent: Successfully completed "
            ...
            current_metadata[
                "technical_agent_executed"
            ] = True

            current_metadata[
                "technical_agent_timestamp"
            ] = str(time.time())

            current_metadata[
                "technical_agent_execution_time"
